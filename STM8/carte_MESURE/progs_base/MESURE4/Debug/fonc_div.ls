...
2891  00b9               L3271:
2892                     ; 73 		displayChar_TFT(col+11*i, ligne, mot[i], ST7735_RED, ST7735_BLACK, 2);
...
2946  00f7 81            	ret
3026                     ; 78 void affiche_nombre(uint16_t nombre, uint8_t col, uint8_t ligne)
3026                     ; 79 {
3027                     	switch	.text
...
3059                     ; 84 	displayChar_TFT(col, ligne, CEN+0x30, 0xFFE0, 0x0000, 2);
...
3079                     ; 85 	displayChar_TFT(col+12, ligne, DIZ+0x30, 0xFFE0, 0x0000, 2);
...
3100                     ; 86 	displayChar_TFT(col+24, ligne, UNI+0x30, 0xFFE0, 0x0000, 2);
...
3267                     ; 107 	CLK_PCKENR1 |= (1<<3);
3269  01c1 721650c7      	bset	_CLK_PCKENR1,#3
3270                     ; 108 	UART2_BRR1 = (uartdiv >> 4);
...
3280                     ; 109 	UART2_BRR2 = ((uartdiv&0xF000)>>8)|(uartdiv&0x000F);
...
3309                     ; 110 	UART2_CR1 = 0;
3311  01f5 725f5244      	clr	_UART2_CR1
3312                     ; 111 	UART2_CR2 = 0b00001100;
3314  01f9 350c5245      	mov	_UART2_CR2,#12
3315                     ; 112 	UART2_CR3 = (UART2_CR3 & 0b10000000);
...
3324  0207 81            	ret
3360                     ; 115 void write_byte_UART2(uint8_t data)
3360                     ; 116 {
3361                     	switch	.text
3362  0208               _write_byte_UART2:
3366                     ; 117 	UART2_DR = data;
3368  0208 c75241        	ld	_UART2_DR,a
3370  020b               L1502:
3371                     ; 118 	while((UART2_SR&0b10000000)==0);
3373  020b c65240        	ld	a,_UART2_SR
3374  020e a580          	bcp	a,#128
3375  0210 27f9          	jreq	L1502
3376                     ; 119 }
3379  0212 81            	ret
3412                     ; 121 void init_timer2_pwm(void)
3412                     ; 122 {
3413                     	switch	.text
3414  0213               _init_timer2_pwm:
3418                     ; 123 	CLK_PCKENR1 = CLK_PCKENR1|(1<<5);
3420  0213 721a50c7      	bset	_CLK_PCKENR1,#5
3421                     ; 124 	TIM2_PSCR = 0x4;
3423  0217 3504530c      	mov	_TIM2_PSCR,#4
3424                     ; 125 	TIM2_ARRH = 99/256;
3426  021b 725f530d      	clr	_TIM2_ARRH
3427                     ; 126 	TIM2_ARRL = 99%256;
3429  021f 3563530e      	mov	_TIM2_ARRL,#99
3430                     ; 127 	TIM2_CCMR1 = 0x68;
3432  0223 35685305      	mov	_TIM2_CCMR1,#104
3433                     ; 128 	TIM2_CCER1 = 1;
3435  0227 35015308      	mov	_TIM2_CCER1,#1
3436                     ; 129 	TIM2_CR1 = 0x81;
3438  022b 35815300      	mov	_TIM2_CR1,#129
3439                     ; 130 	PD_DDR = PD_DDR|(1<<4);
3441  022f 72185011      	bset	_PD_DDR,#4
3442                     ; 131 	PD_CR1 = PD_CR1|(1<<4);
3444  0233 72185012      	bset	_PD_CR1,#4
3445                     ; 132 	PD_CR2 = PD_CR2&~(1<<4);
3447  0237 72195013      	bres	_PD_CR2,#4
3448                     ; 133 }
3451  023b 81            	ret
3518                     ; 136 uint16_t read_AD7991(uint8_t octet_conf)
3518                     ; 137 {
3519                     	switch	.text
3520  023c               _read_AD7991:
3522  023c 88            	push	a
3523  023d 5204          	subw	sp,#4
3524       00000004      OFST:	set	4
3527                     ; 141 	Start_I2C();
3529  023f cd0000        	call	_Start_I2C
3531                     ; 142 	Write_I2C(0b01010000);
3533  0242 a650          	ld	a,#80
3534  0244 cd0000        	call	_Write_I2C
3536                     ; 143 	Write_I2C(octet_conf);
3538  0247 7b05          	ld	a,(OFST+1,sp)
3539  0249 cd0000        	call	_Write_I2C
3541                     ; 144 	Stop_I2C();
3543  024c cd0000        	call	_Stop_I2C
3545                     ; 145 	Start_I2C();
3547  024f cd0000        	call	_Start_I2C
3549                     ; 146 	Write_I2C(0b01010001);
3551  0252 a651          	ld	a,#81
3552  0254 cd0000        	call	_Write_I2C
3554                     ; 147 	Ack_I2C();
3556  0257 cd0000        	call	_Ack_I2C
3558                     ; 148 	adc_MSB = Read_I2C();
3560  025a cd0000        	call	_Read_I2C
3562  025d 6b01          	ld	(OFST-3,sp),a
3563                     ; 149 	NoAck_I2C();
3565  025f cd0000        	call	_NoAck_I2C
3567                     ; 150 	adc_LSB = Read_I2C();
3569  0262 cd0000        	call	_Read_I2C
3571  0265 6b02          	ld	(OFST-2,sp),a
3572                     ; 151 	Stop_I2C();
3574  0267 cd0000        	call	_Stop_I2C
3576                     ; 152 	adc_temp = ((adc_MSB&0b00001111)<<8)+ adc_LSB;
3578  026a 7b01          	ld	a,(OFST-3,sp)
3579  026c a40f          	and	a,#15
3580  026e 5f            	clrw	x
3581  026f 97            	ld	xl,a
3582  0270 4f            	clr	a
3583  0271 02            	rlwa	x,a
3584  0272 01            	rrwa	x,a
3585  0273 1b02          	add	a,(OFST-2,sp)
3586  0275 2401          	jrnc	L24
3587  0277 5c            	incw	x
3588  0278               L24:
3589  0278 02            	rlwa	x,a
3590  0279 1f03          	ldw	(OFST-1,sp),x
3591  027b 01            	rrwa	x,a
3592                     ; 153 	return adc_temp;
3594  027c 1e03          	ldw	x,(OFST-1,sp)
3597  027e 5b05          	addw	sp,#5
3598  0280 81            	ret
3611                     	xdef	_read_AD7991
3612                     	xdef	_init_timer2_pwm
3613                     	xdef	_write_byte_UART2
3614                     	xdef	_init_UART2
3615                     	xdef	_init_timer1_2ms
3616                     	xdef	_affiche_nombre
3617                     	xdef	_affiche_mot
3618                     	xdef	_init_LED
3619                     	xdef	_init_Poussoirs
3620                     	xdef	_init_ADC
3621                     	xdef	_init_SPI
3622                     	xref	_Stop_I2C
3623                     	xref	_NoAck_I2C
3624                     	xref	_Ack_I2C
3625                     	xref	_Read_I2C
3626                     	xref	_Write_I2C
3627                     	xref	_Start_I2C
3628                     	xref	_displayChar_TFT
3629                     	xref.b	c_lreg
3630                     	xref.b	c_x
3649                     	xref	c_ldiv
3650                     	xref	c_rtol
